...
function k(b){"error"===b?(b="error",a(".overlay, .prompt-box").delay(900).fadeIn(500),a(".prompt-box .show-error").show(),a(".prompt-box .show-error p").remove(),a(".prompt-box .error-msg").after('<p class="prompt-error">undefined</p>'),a(".prompt-box .show-login").hide()):b="done";a(".alert").removeClass("busy").addClass(b).delay(800).fadeOut(800,function(){a(this).removeClass(b)})}var h;a(function(){a(".tb_upgrade_plugin").on("click",function(b){b.preventDefault();a(".themify-builder-upgrade-plugin").removeClass("themify-builder-upgrade-plugin");
h=a(this).addClass("themify-builder-upgrade-plugin");e()});(function(){var b=d.location.search;-1<b.indexOf("tfplugin")&&(b=b.match(/tfplugin=([^&,#]+)/),b[1]&&(b=b[1],a(".tb_upgrade_plugin[data-plugin*="+b+"]").trigger("click")))})();a(".tb_upgrade_login").on("click",function(b){b.preventDefault();if(a(".prompt-box").hasClass("update-plugin")){var c=a(this);b=c.parent().parent().find(".username").val();var d=c.parent().parent().find(".password").val(),c=c.closest(".notifications").find(".update").hasClass("login");
""!==b&&""!==d?(g(),a(".alert").addClass("busy").fadeIn(800),a.post(ajaxurl,{action:"themify_builder_validate_login",type:"plugin",login:c,username:b,password:d,nicename_short:h.data("nicename_short"),update_type:h.data("update_type")},function(b){b=a.trim(b);"true"===b?(k(),a("#themify_update_form").append('<input type="hidden" name="plugin" value="'+h.data("plugin")+'" /><input type="hidden" name="package_url" value="'+h.data("package_url")+'" />').submit()):"unsuscribed"===b?(k("error"),e("unsuscribed")):
(k("error"),e("error"))})):(k("error"),e("error"))}});a(".overlay").on("click",function(){g()})})})(jQuery,window,document);
